...
class MainBackend:
    ...
    @st.cache_data
    def generate_prompt_with_user_question_fin(_self, uploaded_file_path, question,selected_company):
        # Set company information params
        if selected_company in company_data:
            company_info = company_data[selected_company]
            company_name = company_info["name"]
    ...
    @st.cache_data
    def generate_response_for_user_question(_self, uploaded_report_path, question,selected_company,status):
        if status=="sus":
        ...
        if status=="fin":
            response = _self.model.completions.create(
                model="claude-2",
                max_tokens_to_sample=400000,
                prompt=_self.generate_prompt_with_user_question_fin(uploaded_report_path, question,selected_company)
                )
